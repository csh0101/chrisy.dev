...
- 业务既有 Kubernetes Pod + PVC 方式，也有直接在机器上启动训练进程的方式。
...
<Table
    data={[
        ...
        { item: '业务使用方式', value: 'Kubernetes Pod + PVC；裸机 / 宿主机直接训练' },
...
- 训练任务：既可以通过 Kubernetes Pod + PVC 访问，也可以在机器上直接读取 JuiceFS 挂载点。
...
第二类是直接运行在机器上的训练任务。很多存量训练脚本不是一开始就容器化的，也不一定通过 PVC 获取数据。为了兼容这些任务，每台训练节点上会常驻一个 JuiceFS Client，把文件系统挂载到机器上的固定路径，训练进程直接读这个挂载点。

所以这套方案不是只服务 Kubernetes，也不是只服务裸机。它更像是一层统一的数据访问能力：上层训练任务可以是 Pod，也可以是机器上的进程；底层都通过同一个 Cache Group 访问远端训练数据。
...
在架构篇里，我们不把启动方式作为主角，但它决定了裸机训练方式能不能稳定运行。对于混合使用 Pod + PVC 和裸机训练的环境，这类细节会直接影响用户体验。

## 小结

这套架构的核心，是把远端对象存储上的训练数据前移到边缘训练集群内部。

Cache Group 使用训练节点上的 NVMe 组成百 TB 级分布式缓存池，Client 通过 `--cache-group=<name>` 和 `--no-sharing` 作为访问端接入缓存组。Kubernetes 任务通过 Pod + PVC 使用，裸机任务通过节点上的常驻 JuiceFS Client 使用。训练前由平台创建 WarmUp CR，把数据预热进 Cache Group，减少训练运行阶段对远端对象存储的直接依赖。
